...
namespace DefList.SubsetStx
  ...
  :=
    (unIrDistElimR (irI subUnR em.unSymm)).unMonoSubR sub
  ...
  :=
    let subInst i:
      dl.SubsetStx
        e.lift.lift
        (instantiateVar ((e.lift.lift 1).lift 1) (var i))
    :=
      lift_lift_eq_one e ▸
            ...
            (arbUnVarI
              (irLR
                (someMonoSub
                  subId
                  (irI
                    (irCtxR (pairMonoSub subId subId anyI))
                    (irCtxL (subInst 1))))
                subId)))
            ...
            (arbUnVarI
              (irLR
                (someMonoSub
                  subId
                  (irI
                    (irCtxR (pairMonoSub subId anyI subId))
                    (irCtxL (subInst 0))))
                subId)))
          (someI (irLR sub.toLift.toLift subId))))
