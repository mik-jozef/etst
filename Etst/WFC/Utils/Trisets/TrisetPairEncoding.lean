/-
  ...
  A `TrisetPair` triset is coded by `(1, i)` where `i` is an index
  of `uniSetMap`, while a `Triset` element is coded by `i` directly.
  To embed the latter into the former, each triset `t` must be mapped
  to a triset `encode t` whose members are the encodings `(1, encode e)`
  of the re-encoded members of `t`:
  
      uniSetMap.call (encode t)
        =
      { (1, encode e) | e ∈ uniSetMap.call t }
  
  (lane-wise). This file defines such an `encode` within WFC.
-/
...
namespace trisetPairEncDl
  ...
  def encOp_ins_def {iFn: Pair}:
    (vals.encOp.call iFn).defMem
      (TrisetPairEncoding.encStepIndex.callEnc iFn)
  :=
    let inCallEncWfm:
      (trisetPairEncDl.wfm consts.callEnc).defMem
        (.pair TrisetPairEncoding.encStepIndex
          (.pair iFn (TrisetPairEncoding.encStepIndex.callEnc iFn)))
    :=
      wfm_callEnc_eq ▸
        opFixDl.callEnc_ins TrisetPairEncoding.encStepIndex iFn
    let inCallEnc:
      ((BasicExpr.const consts.callEnc).toLane .defLane).intp2
        [.pair iFn (TrisetPairEncoding.encStepIndex.callEnc iFn),
         TrisetPairEncoding.encStepIndex.callEnc iFn,
    ...
    inProd (inVar rfl)
      (inCall
        (pA := iFn)
        (pB := TrisetPairEncoding.encStepIndex.callEnc iFn)
        (inCall
          (pA := TrisetPairEncoding.encStepIndex)
          (pB := .pair iFn (TrisetPairEncoding.encStepIndex.callEnc iFn))
          inCallEnc
          (inToExpr TrisetPairEncoding.encStepIndex))
        (inVar rfl))
  
  /-- The possible-membership variant of `encOp_ins_def`. -/
  def encOp_ins_pos {iFn: Pair}:
    (vals.encOp.call iFn).posMem
      (TrisetPairEncoding.encStepIndex.callEnc iFn)
  :=
    let inCallEncWfm:
    ...
    :=
      wfm_callEnc_eq ▸
        (opFixDl.callEnc_ins TrisetPairEncoding.encStepIndex iFn).toPos
    let inCallEnc:
      ((BasicExpr.const consts.callEnc).toLane .posLane).intp2
        [.pair iFn (TrisetPairEncoding.encStepIndex.callEnc iFn),
         TrisetPairEncoding.encStepIndex.callEnc iFn,
  ...
  :
    s = TrisetPairEncoding.encStepIndex.callEnc iFn
  :=
    ...
    let h2 :=
      inCallElimSingle
        h1
        (intp2_toExpr_eq_singleton TrisetPairEncoding.encStepIndex)
    let h2w:
      (trisetPairEncDl.wfm consts.callEnc).defMem
        (.pair TrisetPairEncoding.encStepIndex (.pair iFnA s))
    :=
      h2
    let encEq :=
  ...
  def encOp_call_eq (iFn: Pair):
    Eq
      (vals.encOp.call iFn)
      (Set3.just (TrisetPairEncoding.encStepIndex.callEnc iFn))
  :=
    Set3.eq4
...
namespace TrisetPairEncoding
  ...
  def encodeFixIndex: Pair := indexFixIndex encOpIndex
  
  /-- The fixed point equation of `encodeFixIndex`. -/
  def encodeFixIndex_call_eq:
    uniSetMap.call encodeFixIndex
      =
    uniSetMap.call (encStepIndex.callEnc encodeFixIndex)
  :=
  ...
  /--
    The re-encoding map on triset indices: `encode t` is a
    triset whose members are the encodings `(1, encode e)` of
    the re-encoded members of `t`.
  -/
  def encode (t: Pair): Pair := encodeFixIndex.callEnc t
  
  /-- The characteristic equation of `encode`. -/
  def encode_call_eq (t: Pair):
    uniSetMap.call (encode t)
      =
    Set3.image (fun e => .pair (.nat 1) (encode e)) (uniSetMap.call t)
  :=
    (opFixDl.callEnc_correct encodeFixIndex t).trans
      ((congrArg (·.call t) encodeFixIndex_call_eq).trans
        ((congrArg
            (·.call t)
            (opFixDl.callEnc_correct encStepIndex encodeFixIndex)).trans
          ((congrArg
              (fun s => (s.call encodeFixIndex).call t)
              encStepIndex_call).trans
            (trisetPairEncHelpersDl.encStep_call_eq encodeFixIndex t))))
  
  /-- Definitive members of `t` encode to definitive members of
  `encode t`. -/
  def encode_ins {t e: Pair}
    (ins: (uniSetMap.call t).defMem e)
  :
    (uniSetMap.call (encode t)).defMem (.pair (.nat 1) (encode e))
  :=
    encode_call_eq t ▸ ⟨e, ins, rfl⟩
  
  /-- Possible members of `t` encode to possible members of
  `encode t`. -/
  def encode_inw {t e: Pair}
    (inw: (uniSetMap.call t).posMem e)
  :
    (uniSetMap.call (encode t)).posMem (.pair (.nat 1) (encode e))
  :=
    encode_call_eq t ▸ ⟨e, inw, rfl⟩
  
  /-- Definitive members of `encode t` are encodings of definitive
  members of `t`. -/
  def encode_elim_def {t p: Pair}
    (mem: (uniSetMap.call (encode t)).defMem p)
  :
    ∃ e, (uniSetMap.call t).defMem e ∧ p = .pair (.nat 1) (encode e)
  :=
    let ⟨e, hin, heq⟩ := encode_call_eq t ▸ mem
    ⟨e, hin, heq.symm⟩
  
  /-- Possible members of `encode t` are encodings of possible
  members of `t`. -/
  def encode_elim_pos {t p: Pair}
    (mem: (uniSetMap.call (encode t)).posMem p)
  :
    ∃ e, (uniSetMap.call t).posMem e ∧ p = .pair (.nat 1) (encode e)
  :=
    let ⟨e, hin, heq⟩ := encode_call_eq t ▸ mem
    ⟨e, hin, heq.symm⟩
