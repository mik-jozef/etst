...
namespace SingleLaneExpr
  ...
  /-
    This is not a mistake -- the variables of the domain are updated
  ...
  def inZthFstElim
    (inZth: intp2 (zthMember (Expr.df lane x)) bv b c zth)
    (inFst: intp2 (fstMember (Expr.df lane x)) bv b c fst)
    (isUnit: c x = Set3.just d)
  :
    intp2 (Expr.df lane x) bv b c (Pair.pair zth fst)
  :=
  ...
  def inCallElimDef
    (inCall: intp2 (call fn (Expr.df lane arg)) bv b c pB)
