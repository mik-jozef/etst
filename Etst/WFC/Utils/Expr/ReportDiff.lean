...
:=
  ...
  | .un _ _, .prod _ _ => s!"{path}: un vs prod"
  ...
  | .some _, .prod _ _ => s!"{path}: some vs prod"
  ...
  | .arbUn _, .prod _ _ => s!"{path}: arbUn vs prod"
  ...
  | .var xA, .var xB =>
    if xA != xB then
      ...
      none
  | .null, .null => none
  | .prod aLeft aRite, .prod bLeft bRite =>
    match reportDiff aLeft bLeft (path ++ ".prodL") with
    | some diff => some diff
    | none => reportDiff aRite bRite (path ++ ".prodR")
  ...
  | .const _ _, .prod _ _ => s!"{path}: const vs prod"
  ...
  | .var _, .prod _ _ => s!"{path}: var vs prod"
  ...
  | .null, .prod _ _ => s!"{path}: null vs prod"
  ...
  | .prod _ _, .un _ _ => s!"{path}: prod vs un"
  | .prod _ _, .some _ => s!"{path}: prod vs some"
  | .prod _ _, .arbUn _ => s!"{path}: prod vs arbUn"
  | .prod _ _, .const _ _ => s!"{path}: prod vs const"
  | .prod _ _, .var _ => s!"{path}: prod vs var"
  | .prod _ _, .null => s!"{path}: prod vs null"
  | .prod _ _, .ir _ _ => s!"{path}: prod vs ir"
  | .prod _ _, .full _ => s!"{path}: prod vs full"
  | .prod _ _, .compl _ => s!"{path}: prod vs compl"
  | .prod _ _, .arbIr _ => s!"{path}: prod vs arbIr"
  ...
  | .ir _ _, .prod _ _ => s!"{path}: ir vs prod"
  ...
  | .full _, .prod _ _ => s!"{path}: full vs prod"
  ...
  | .compl _, .prod _ _ => s!"{path}: compl vs prod"
  ...
  | .arbIr _, .prod _ _ => s!"{path}: arbIr vs prod"
