...
structure BackgroundCause (D: Type*) where
  backgroundIns: Set (ValDef D)
  backgroundOut: Set (ValDef D)
...
/-
  ...
  We encode the causes as sets of `ValDef` instances. A cause
...
/-
  ...
  for every value–definition pair `(d, x)`:
...
/-
  ...
  definition pairs if for some element (d, x) of S, either:
  ...
  A set of value–definition pairs is called an empty cycle if all
