...
namespace DefList
  ...
  | defPos: dl.IsFullStx (impl (df .defLane x) (df .posLane x))
    -- TODO should be provable using induction.
  | unfold (lane: Set3.Lane) (x: Nat):
      dl.IsFullStx (impl (df lane x) ((dl.getDef x).toLane lane))
  | fold (lane: Set3.Lane) (x: Nat):
      dl.IsFullStx (impl ((dl.getDef x).toLane lane) (df lane x))
  | mp
  ...
  | mutInduction {x}
    ...
    :
      dl.IsFullStx
        (impl x (condFull (impl (df desc[i].lane desc[i].x) desc[i].expr)))
  ...
  namespace IsFullStx
    ...
    def mutInductionDirect
      ...
      (i: desc.Index)
    :
      IsFullStx dl (impl (df desc[i].lane desc[i].x) desc[i].expr)
    :=
