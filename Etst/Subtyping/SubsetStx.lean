...
:=
  match e with
  | df i x => if ed then replacer depth i x else .df i x
  | var x => .var x
  | null => null
  | pair left rite =>
      pair
...
-- Represents an inductive proof of `df lane x ⊆ expr`
...
:
  SingleLaneExpr
:=
  desc.foldr (InductionDescriptor.hypothesis depth lane x) (df lane x)
...
|
  subDefPos {x}:
    dl.SubsetStx ctx (df .defLane x) (df .posLane x)
|
  pairMono
...
|
  ...
  :
    dl.SubsetStx
      ctx
      x
      (condFull (impl (df desc[i].lane desc[i].x) desc[i].expr))
...
namespace DefList.SubsetStx
  ...
  def unfoldCtx
      (sub: dl.SubsetStx ctx (df lane a) b)
    :
  ...
  def unfold
      (sub: dl.SubsetStx ctx x (df lane a))
    :
  ...
  def subMutInduction
    ...
    (i: desc.Index)
  :
    dl.SubsetStx ctx (df desc[i].lane desc[i].x) desc[i].expr
  :=
    isFullImplElim
  ...
  def subInduction
    ...
    (premise:
      dl.SubsetStx
        ...
        desc.expr)
  :
    dl.SubsetStx ctx (df desc.lane desc.x) desc.expr
  :=
    subMutInduction
      [desc]
      (fun | ⟨0, _⟩ => premise)
    ...
    (premise:
      dl.SubsetStx
        ...
        (((dl.getDef x).toLane lane).replaceDepthEvenVars 0 true fun depth l xR =>
          if l.Le lane && x = xR then .ir (expr.lift 0 depth) (df l xR) else (df l xR))
        expr)
  :
    dl.SubsetStx ctx (df lane x) expr
  :=
    subInduction
      {
        x,
