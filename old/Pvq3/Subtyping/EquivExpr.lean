import old.Pvq3.Subtyping.OfSubExpr
import old.Pvq3.Syntax.PExpr
...
namespace Pvq3.Desirables
  ...
  /-
    ...
    TODO Equivalence under the transformation towards the prenex normal
    form.
    
    TODO equivalence under the same infinite expansion of variables
      atually no that's too strong. Just equivalence under
      - splitting a definition into two,
      - merging two definitions,
      - I would like to add something about transformations that
      - do not change the "density" of variables, but not sure
      - if that ought to be covered.
  -/
